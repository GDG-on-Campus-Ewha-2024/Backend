...
class Post(models.Model):
  """블로그 게시글 모델"""
  title = models.CharField(max_length=200, blank=False) # 제목
  content = models.TextField() # 내용
  published_date = models.DateField(auto_now_add=True) # 게시물 작성 시간 
  
  def __str__(self):
     # 관리자 페이지나 Django 쉘에서 Post 객체를 볼 때 제목을 표시
    return self.title 